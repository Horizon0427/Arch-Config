...
int main(void) {
  ...
  if ((dir = opendir(wp_dir)) != NULL) {
    while ((ent = readdir(dir)) != NULL) {
      if (HasExtension(ent->d_name, ".png") ||
        ...
        if (wpCount < MAX_WALLPAPERS) {
          BeginDrawing();
          ClearBackground(BLANK);
          DrawText("Loading High-Res Wallpapers...", GetScreenWidth() / 2 - 250,
          ...
          Image img = LoadImage(full_img_path); // 读取绝对路径下的图
          if (img.width > 1) {
            ...
            strcpy(wallpapers[wpCount].filename,
                   ent->d_name); // 仅保存文件名即可
  ...
  while (!WindowShouldClose()) {
    ...
    float startY = (GetScreenHeight() - totalHeight) / 2.0f + HEX_RADIUS;
    ...
    if (hoveredIndex != -1) {
      ...
      if (IsMouseButtonReleased(MOUSE_BUTTON_LEFT)) {
        ...
        snprintf(cmd, sizeof(cmd),
                 "("
                 "awww img \"%s\" --transition-type any --transition-angle 30 "
